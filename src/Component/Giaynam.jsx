...
const Giaynam = () => {
  ...
  const sandal = Categories.filter((item) => item.title === "giày nam hot");
  const sandalheading = sandal.map((item) => item.title);
  const sandalfilter = API.filter((item)=>item.slug === 'giay-nam-hot')
  return (
    <section className="sandalnam">
