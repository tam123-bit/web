...
const Giaychieucao = () => {
  ...
  const sandal = Categories.filter((item) => item.title === "giày tăng chiều cao");
  const sandalheading = sandal.map((item) => item.title);
  const sandalfilter = API.filter((item)=>item.slug === 'giay-tang-chieu-cao')
  return (
    <section className="sandalcao">
